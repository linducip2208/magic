    ...
    >
		<?php
			...
			if (isset($item->avatar)) {
				if (str_starts_with($item->avatar, 'asset')) {
					$avatarSrc = custom_theme_url($item->avatar);
				} else {
					$avatarSrc = url('') . '/testimonialAvatar/' . $item->avatar;
				}
			}
		?>
		...
		<img
			...
			alt="<?php echo e(__($item->full_name)); ?>"
		>
    </figure>
    <div class="whitespace-nowrap opacity-0 transition-all group-[&.is-nav-selected]:opacity-100">
        <p class="text-heading-foreground"><?php echo __($item->full_name); ?></p>
        <p class="text-heading-foreground opacity-15"><?php echo __($item->job_title); ?></p>
    </div>
</div>
<?php /**PATH D:\project laravel\magicai\resources\views/default/landing-page/testimonials/item-image.blade.php ENDPATH**/ ?>