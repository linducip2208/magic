...
@section('content')
   ...
   {!! adsense_chat_pro_top_header_728x90() !!}
    ...
    >
        ...
        <div class="pointer-events-none absolute top-52 hidden size-52 -translate-x-2/3 bg-rose-500 opacity-10 blur-3xl transition-all dark:opacity-5 lg:block"></div>
        ...
        <div class="grow px-2 md:px-5 lg:px-0">
            <div @class([
                'relative end-0 start-0 top-0 z-40 h-[--header-h] justify-between gap-3 bg-background px-3.5 transition-all md:px-5 lg:absolute lg:start-[--sidebar-w] lg:z-10 xl:px-8',
            ...
            <div class="max-lg:py-5">
                ...
                >
                    <div
                        ...
                        :class="{ active: mobileSidebarShow || !sidebarHidden, hidden: $store.focusMode.active && sidebarHidden }"
                        @click.outside="(e) => {return !sidebarHidden && !IsShowRecent(e?.target) && (sidebarHidden = true);}"
                    >
                        @if (view()->hasSection('chat_sidebar'))
                            @yield('chat_sidebar')
                        @else
                            @include('panel.user.openai_chat.components.chat_sidebar', ['website_url' => 'chatpro'])
                        @endif
    ...
    <template id="chat_ai_bubble">
        <div class="lqd-chat-ai-bubble group mb-2 flex content-start items-start gap-2">
            <div class="lqd-chat-sender flex items-center gap-2.5">
                <span
